...
function updateCountdown() {
  ...
  if (!navbarCountdown || !mainCountdown) return;

  if (diff <= 0) {
    navbarCountdown.textContent = "LAUNCHED";
    mainCountdown.textContent = "LAUNCHED";
    return;
  }
  ...
  const formatted = `T-${String(days).padStart(2, "0")}D ${String(hours).padStart(
    2,
    "0"
  )}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;

  navbarCountdown.textContent = formatted;
  mainCountdown.textContent = formatted;
}
setInterval(updateCountdown, 1000);
updateCountdown();
